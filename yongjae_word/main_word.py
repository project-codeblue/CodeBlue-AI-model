...
stopwords = ['입원','나','면서','씩','증도','닮다','다','없다','고','엔','힘들다','액체','까지','두','야하다','오늘','아침','오늘아침','커지다','급격하다','치가','나타나다','조','이상승','이송이','병원','력','지속','함피','함','즉시','집중','떨어지다','하고','심하다','인','적','며','필요하다','되어다','되었다','인하다','심해지다','계속','있다','심각하다','점점','때문','을','로','상','하','지다','않다','이다','되다','에서','가','이','은','들','는','좀','잘','걍','과','도','를','으로','자','에','와','한','하다','아침','또','또한','가끔','감','코',"계,'속",'현재','이어진','상황']
    
def preprocess(text):
    spacing_line = spacing(text)
    print("첫번쨰 spacing",spacing_line)
    # spacing_line = re.compile('[^가-힣]').sub('', spacing_line)
    # print("두번쨰 spacing",spacing_line)

    if spacing_line:
        token_text = spacing_line.split()
        print("1차 가공 단어 ==",token_text)
    ...
    for word in preprocessed_words:
        ...
        #     break
            ...
            if max_similarity >= 0.93:
                ...
                #총 점수에서 "없음""없다"라는 부정어가 들어가면 평균점수라도 빼는게어떨까?
